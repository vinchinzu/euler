...
fn main() {
    ...
    // Step 1: Pre-generate all LCG values sequentially (fast)
    ...
    // Step 2: Compute ranks in parallel using thread-local scratch structs
    thread_local! {
        static SCRATCH: RefCell<RankScratch> = RefCell::new(RankScratch::new());
    }

    let mut ranks: Vec<u64> = a_vals
        .par_iter()
        .map(|&a_val| {
            SCRATCH.with(|s| {
                let mut scratch = s.borrow_mut();
                let mut w = [0i32; N + 1];
                int_to_word12(a_val, &mut w);
                scratch.rank_db(&w) as u64
            })
        })
        .collect();
