...
fn main() {
    ...
    // Collect primes, power in parallel (expensive pow_mod)
    let primes: Vec<usize> = (2..=k).filter(|&i| spf[i] == i as u32).collect();
    let prime_pows: Vec<i64> = primes
        .par_iter()
        .map(|&p| power_mod(p as i64, n + 1, MOD))
        .collect();
    ...
    for i in 2..=k {
        if spf[i] != i as u32 {
            pows[i] = (pows[spf[i] as usize] as i128 * pows[i / spf[i] as usize] as i128
                % MOD as i128) as i64;
        }
    }
    ...
    invs[1] = 1;
    for i in 2..=k {
        invs[i] = (MOD - (MOD / i as i64) * invs[(MOD % i as i64) as usize] % MOD) % MOD;
    }
    ...
    for t in 0..k {
        let num_words = if t == 0 {
            ...
            (pows[t] - 1 + MOD) % MOD * invs[t - 1] % MOD
        };
        ans = (ans + (num_words as i128 * inner_sum as i128 % MOD as i128) as i64) % MOD;
        if t < k - 1 {
            let new_ncr = (ncr as i128 * ((k - t) as i64 % MOD) as i128 % MOD as i128
                * invs[t + 1] as i128
                % MOD as i128) as i64;
            let parity_val = if (k - t) % 2 == 0 { 1i64 } else { MOD - 1 };
            inner_sum = (inner_sum
                + 1
                + (parity_val as i128 * ((ncr + new_ncr) % MOD) as i128 % MOD as i128) as i64)
                % MOD;
