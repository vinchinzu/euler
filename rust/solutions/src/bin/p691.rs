...
unsafe fn match_len(s: &[u8], i: usize, j: usize, mut h: usize, n: usize) -> usize {
    // SAFETY: callers keep i, j < n; wide loads stay inside [0, n).
    unsafe {
        let sp = s.as_ptr();
        while h + 16 <= n.wrapping_sub(i) && h + 16 <= n.wrapping_sub(j) {
            let a = std::ptr::read_unaligned(sp.add(i + h) as *const u128);
            let b = std::ptr::read_unaligned(sp.add(j + h) as *const u128);
            if a != b {
                let xor = a ^ b;
        ...
        while h + 8 <= n.wrapping_sub(i) && h + 8 <= n.wrapping_sub(j) {
            let a = std::ptr::read_unaligned(sp.add(i + h) as *const u64);
            let b = std::ptr::read_unaligned(sp.add(j + h) as *const u64);
            if a != b {
                let xor = a ^ b;
                let z = if cfg!(target_endian = "little") {
                    xor.trailing_zeros()
...
// parent[i] == INACTIVE: not yet activated.
// parent[root] < 0: -size of component.
// parent[i] >= 0: parent index (path-compressed).
const INACTIVE: i32 = i32::MAX;

#[inline(always)]
fn find(parent: &mut [i32], mut x: i32) -> i32 {
    // SAFETY: x is an activated UF index in 0..n; parent has length n.
    unsafe {
        let p0 = parent.as_mut_ptr();
        let mut p = *p0.add(x as usize);
        if p < 0 {
            return x;
        }
        loop {
            let gp = *p0.add(p as usize);
            if gp < 0 {
                *p0.add(x as usize) = p;
                return p;
            }
            *p0.add(x as usize) = gp;
            x = p;
            p = gp;
        }
    }
...
#[inline(always)]
fn unite(parent: &mut [i32], a: i32, b: i32, global_max_sz: &mut i32) {
    ...
    unsafe {
        let p0 = parent.as_mut_ptr();
        let mut sa = -*p0.add(a as usize);
        let mut sb = -*p0.add(b as usize);
        if sa < sb {
        ...
        *p0.add(b as usize) = a;
        let new_sz = sa + sb;
        *p0.add(a as usize) = -new_sz;
        if new_sz > *global_max_sz {
            *global_max_sz = new_sz;
        }
    }
}
...
fn main() {
    ...
    let mut parent = vec![INACTIVE; n];

    let mut global_max_sz = 0i32;
    let mut ans = big_n as i64;
    let mut max_freq = 1i32;
    let big_n_i = big_n as i32;
    ...
    for v in (1..=ml).rev() {
        ...
        unsafe {
            let idx_p = idx_buf.as_ptr();
            let p0 = parent.as_mut_ptr();
            for jj in si..ei {
                let i = *idx_p.add(jj) as usize;
                *p0.add(i) = -1;
                if global_max_sz == 0 {
                    global_max_sz = 1;
                }
                if i > 1 && *p0.add(i - 1) != INACTIVE {
                    unite(&mut parent, i as i32, (i - 1) as i32, &mut global_max_sz);
                }
                if i + 1 < n && *p0.add(i + 1) != INACTIVE {
                    unite(&mut parent, i as i32, (i + 1) as i32, &mut global_max_sz);
                }
            }
        }

        let freq = (global_max_sz + 1).min(big_n_i);
        if freq > max_freq {
            ans += (freq - max_freq) as i64 * v as i64;
            max_freq = freq;
        }
    }
