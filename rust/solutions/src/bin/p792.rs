...
#[inline(always)]
fn v2_factorial(n: i64) -> i64 {
    n - (n as u64).count_ones() as i64
}

#[inline(always)]
fn oddpart_factorial(mut n: i64, table: &[u64]) -> u64 {
    let mut result = 1u64;
    while n > 0 {
...
#[inline(always)]
fn ncr_mod(a: i64, b: i64, table: &[u64]) -> u64 {
    if b < 0 || b > a {
        return 0;
    }
    let num = oddpart_factorial(a, table);
    let den = (oddpart_factorial(b, table).wrapping_mul(oddpart_factorial(a - b, table))) & MASK_EFF;
...
fn main() {
    ...
    let ans: i64 = (1..=n_val)
        ...
        .map(|m| {
            ...
            let mut res: u64 = 0;
            for k in 0..B_EFF {
                let term = ncr_mod(2 * n + 1, n + k as i64 + 1, &table);
                if k % 2 == 0 {
