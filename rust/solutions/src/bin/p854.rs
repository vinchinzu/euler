...
fn main() {
    ...
    let z_computed: Vec<i32> = small_primes
        .par_iter()
        .map(|&p| get_z_rank(p, &spf))
        .collect();
    let mut z_vals = vec![0i32; LIMIT + 1];
    for (i, &p) in small_primes.iter().enumerate() {
        z_vals[p as usize] = z_computed[i];
    }

    // Flat array: period -> product. Key space is 1..=LIMIT.
    let mut ubp = vec![1i64; LIMIT + 1];

    for &p in &small_primes {
        if p == 2 {
            let mut period = 3i32;
            while period <= LIMIT as i32 {
                ubp[period as usize] = ubp[period as usize] * 2 % MOD;
                period *= 2;
            }
        } else {
            ...
            loop {
                ...
                ubp[idx] = ubp[idx] * p as i64 % MOD;
                curr_z *= p as i64;
                if curr_z > LIMIT as i64 && period * p as i64 > LIMIT as i64 {
                    break;
                }
            }
        }
