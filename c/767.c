 ...
 *
 * Find the number of 16xN matrices with 0/1 entries where every 2xK sub-matrix
 * has exactly K ones. Uses FFT with long double precision for modular convolution.
...
void convolve_mod(int64_t* a, int64_t* b, int64_t* result, int n, int64_t mod) {
    ...
    for (int i = 0; i < 2*n-1; i++) {
        ...
        if (lh < 0) lh += mod;
        if (hh < 0) hh += mod;
        result[i] = (ll + (int128)lh * BASE % mod + (int128)hh * BASE % mod * BASE % mod) % mod;
    }
