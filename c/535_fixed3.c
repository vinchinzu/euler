...
/* Triangular number n*(n+1)/2 mod MOD, handling n up to 10^18 */
static ll tr(ll n) {
    /* n*(n+1)/2 mod MOD. Since one of n, n+1 is even, divide that one by 2 first,
     * then reduce mod MOD, then multiply. */
    ll half, other;
    if (n % 2 == 0) {
        half = (n / 2) % MOD;
        other = (n + 1) % MOD;
