...
A group of chefs take turns in order. On each turn, chef i has probability S_i
of being able to select another chef and removing them from the game. Find the
expected number of turns the game takes if all chefs play optimally.

Uses bitmask DP with linear system solving, matching the Java solution.
"""
...
"""
    ...
    c_file = os.path.join(tmpdir, "p481.c")
    exe_file = os.path.join(tmpdir, "p481")
