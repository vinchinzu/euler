...
Port of Lucy-style DP for sum of x^e * phi(x), embedded C for speed.
Uses O(N^{2/3}) sieve + recursion approach.
"""
...
static ll S_k(int k, ll n) {
    ...
    if (k == 2) {
        /* n*(n+1)*(2n+1)/6 mod M */
        /* Factor: always divisible by 6. Compute carefully. */
        ll a = n, b = n + 1, c = 2*n + 1;
        /* Remove factor of 2 from one of a,b */
        if (a % 2 == 0) a /= 2; else b /= 2;
        /* Remove factor of 3 from one of a,b,c */
        if (a % 3 == 0) a /= 3;
        else if (b % 3 == 0) b /= 3;
        else c /= 3;
...
int main(void) {
    ...
    #define sp_get(e, q) ({ \
        ll _q = (q); \
        ll _r; \
        if (_q <= L) _r = (ll)sm[e][_q]; \
        else _r = (ll)lg[e][N / _q]; \
        _r; \
    })
...
"""
    ...
    src = os.path.join(tmpdir, "sol415.c")
    exe = os.path.join(tmpdir, "sol415")
