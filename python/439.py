...
S(N) = sum_{i=1}^N sum_{j=1}^N d(i*j) where d(k) is sum of divisors of k.
...
// Triangular number mod MOD: n*(n+1)/2
static inline ll tr(ll n) {
    n %= (2 * MOD);
    return (n * (n + 1) / 2) % MOD;
}

// Sum from a to b inclusive: tr(b) - tr(a-1)
...
'''
    ...
    try:
        ...
        result = subprocess.check_output([exe_file], timeout=300).decode().strip()
        return int(result)
    finally:
        os.unlink(c_file)
