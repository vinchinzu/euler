 ...
 *
 * Find the number of 18-polyominoes with a plinth at (0,0), all other blocks
 * with y > 0, and center of mass at x = 0.
 *
 * Algorithm: enumerate column counts, group by (c_0, moment), then for each
 * matching pair use Redelmeier's algorithm to count connected polyominoes.
 */
...
#define N 18
#define MAX_CC 300000  /* upper bound on number of column counts */

/* Directions */
static const int DX[] = {1, -1, 0, 0};
static const int DY[] = {0, 0, 1, -1};

/* Global state for Redelmeier */
...
/* Column count storage */
static int cc_data[MAX_CC][N]; /* each cc is stored as values */
static int cc_len[MAX_CC];     /* length of each cc */
static int num_cc = 0;
...
/* Grouping: key = (c_0, moment), value = list of cc indices */
typedef struct {
    int c0;
    int moment;
} Key;

/* Simple hash map for grouping */
...
int main(void) {
    ...
    for (int h = 0; h < HASH_SIZE; h++) {
        for (Node *node = hash_table[h]; node; node = node->next) {
            ...
            /* For each pair (cc1, cc2) where sum(cc1) + sum(cc2) == N + num_middle */
            /* Group by sum within the group */
            /* ordering[i] = position of group[i] in the group */
            ...
            for (int i1 = 0; i1 < group_size; i1++) {
                ...
                for (int i2 = i1; i2 < group_size; i2++) {
                    ...
                    /* Set left half (cc2, skip index 0) */
