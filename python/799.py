...
Find the smallest pentagonal number P_c that can be expressed as P_a + P_b
in over 100 different ways (a <= b).

Key insight: P_a + P_b = P_c reduces to (6a-1)^2 + (6b-1)^2 = 36c^2 - 12c + 2.
So we need to count representations of n = 36c^2 - 12c + 2 as x^2 + y^2 with
x, y > 0, x <= y, x = 5 mod 6, y = 5 mod 6.

m(c) = 18c^2 - 6c + 1 only has prime factors p = 1 mod 4. We sieve R(c) =
product of (2*e_i + 1) over all prime factors of m(c), then use Gaussian
integer factorization to count filtered representations for high-R candidates.

Two-pass sieve approach:
  Pass 1: Compute R(c) for all c using primes up to 500K.
  Pass 2: For candidates with R >= 1000, record full factorization.
  Then enumerate representations via Gaussian integers.
