...
Compute sum_{n=2}^{31} M(n), where M(n) is the smallest number with the same
binary factor tree shape as n!!.

This script compiles and runs a C solver that follows the exact Java strategy:
- For each n, compute target shape T(n!!).
- Enumerate nondecreasing multisets of exactly k primes (k is the total prime
  exponent count of n!!), with strong branch-and-bound pruning.
- Compare shapes using memoized factor-tree construction.
"""
...
static size_t CACHE_SIZE = 1u << 21;  // 2M slots
