...
Given binary string S generated by specific rules, find sum of L(k,S)
where L(k) is the length of the longest substring appearing at least k times.

Uses suffix array + LCP array approach with union-find for sweeping
LCP thresholds from high to low. Implemented in C for performance.
"""
...
def solve() -> int:
    """Solve Problem 691 using compiled C."""
    c_code = r"""
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void) {
    ...
    /* Counting sort: group indices by LCP value in ascending order.
       Bucket for value v starts at prefix_sum[v].
       We iterate levels from max_lcp down to 1. */
    int *lcp_cnt = (int *)calloc(max_lcp + 2, sizeof(int));
    for (int i = 1; i < n; i++)
        if (lcp_arr[i] > 0) lcp_cnt[lcp_arr[i]]++;

    /* prefix_sum[v] = sum of lcp_cnt[1..v-1] = start position of bucket v */
    ...
    /* Sweep from largest LCP to smallest */
