...
Key insight: S(2r) = 2*S(r) because for n = 4m, all representations
n = x^2+y^2+z^2 have x,y,z all even. So S(10^10) = 2^10 * S(5^10).

This reduces the problem from r=10^10 to r=5^10=9765625, which is tractable.

S(r) = 6r + 24 * sum_{x=1}^{r-1} x * f((r-x)(r+x))
where f(n) = r_2(n)/4 = product over odd prime powers in n.

For r=5^10, gcd(r-x, r+x) | 2*5^10, so the only shared odd prime is 5.
Thus f(a*b) = f_no5(a) * f_no5(b) * (v_5(a) + v_5(b) + 1) where a=r-x, b=r+x.

Uses a sieve over [1, 2*5^10] to compute f_no5 and v_5.
"""
...
"""
    ...
    try:
        subprocess.run(['gcc', '-O2', '-o', exe_file, c_file],
