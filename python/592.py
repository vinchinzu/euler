...
/*
 * Compute the last 12 hex digits before trailing zeros of N! where N = 20!.
 *
 * Algorithm:
 * 1. odd_part(N!) = product of (i / 2^v2(i)) for i=1..N, mod 2^48
 *    Recursion: odd_part(N!) = PO(N) * odd_part(floor(N/2)!)
 *    where PO(N) = product of odd i, 1 <= i <= N
 *
 * 2. PO(N) depends only on r = ceil(N/2) mod 2^47 (period property)
 *
 * 3. Compute PO via BSGS: split r into blocks of size B,
 *    f(a) = product of B consecutive odd numbers is a degree-~24 polynomial mod 2^48
 *    Use forward differences to evaluate at all block indices.
 *
 * 4. v2(N!) = sum floor(N/2^k), then answer = odd_part * 2^(v2 mod 4) mod 2^48
 */
