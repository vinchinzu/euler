...
Count geometric triangles (a<=b<=c, b^2=ac) with perimeter <= 2.5*10^13.
Parametrize as a=q^2*m, b=pq*m, c=p^2*m with gcd(p,q)=1, p>=q.
Uses Mobius inversion for coprimality and staircase method for inner sums.
...
"""
    ...
    src = os.path.join(tmpdir, "sol370.c")
    exe = os.path.join(tmpdir, "sol370")
